...
CREATE TABLE `catroutes` (
    `irouteid` INTEGER NOT NULL AUTO_INCREMENT,
    `dtcreated` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `dtupdated` DATETIME(3) NULL,
    `vroute` VARCHAR(255) NULL,
    `vfeacture` VARCHAR(255) NULL,
    `iactivo` TINYINT NOT NULL DEFAULT 1,

    UNIQUE INDEX `catroutes_vroute_vfeacture_key`(`vroute`, `vfeacture`),
...
-- AddForeignKey
ALTER TABLE `catmenus` ADD CONSTRAINT `catmenus_irouteid_fkey` FOREIGN KEY (`irouteid`) REFERENCES `catroutes`(`irouteid`) ON DELETE RESTRICT ON UPDATE CASCADE;
